...
class Archives extends Model
{
    ...
    {
        return addon_url('cms/archives/index', [':diyname' => $data['diyname']]);
    }
    ...
    protected static function init()
    {
        self::afterInsert(function ($row) {
            ...
            if ($channel)
            {
                ...
                db('channel')->where('id', $channel['id'])->setInc('items');
            }
        });
        self::beforeWrite(function($row) {
            $channelInfo = db('channel')->field('model_id')->where("id={$row->channel_id}")->find();
            $modelInfo = db('model')->field('table')->where("id={$channelInfo['model_id']}")->find();
            $row->append_table = $modelInfo['table'];
            //在更新之前对数组进行处理
        ...
        self::afterDelete(function ($row) {
            ...
            if ($channel)
            {
                db('channel')->where('id', $channel['id'])->setDec('items');
            }
        });
    ...
    public function getFlagTextAttr($value, $data)
    {
        ...
        $valueArr = $value ? $value : explode(',', $value);
